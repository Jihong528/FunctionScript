...
cellInfo <- data.frame(seuratCluster=P2@meta.data$orig.ident) #CellType
colnames(cellInfo) <- "CellType"
rownames(cellInfo) <- rownames(P2@meta.data)
...
runSCENIC_3_scoreCells(scenicOptions, exprMat_filtered_log) #?????????????߳?,?ĳ?nCores=1
